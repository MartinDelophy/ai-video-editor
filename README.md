...
- **August 31, 2026 — Faster large-video preparation:** local imports decode an adaptive first-pass filmstrip sized to the computer instead of always paying the full 480-frame cost. Online-library videos now show real download progress before switching to frame preparation, use a bounded editing rendition, and capture the true opening frame so the filmstrip has no artificial black lead-in. The original Blob is reused without copying hundreds of megabytes; exact thumbnails fill the visible viewport first, then appear across offscreen time in midpoint-bisection order, while the playhead cell follows the live preview frame.
