...
- **2026-09-26** — Sanity 分镜工作台：新增可选的脚本与分镜编辑、带版本校验的审核批准，以及匹配本地素材后追加至时间线的流程；分镜备注保持可编辑，编辑器继续以本地处理为主，新控件覆盖 13 种语言。 部署版工作台采用 Astro，复用 React 审核组件。
- **2026-09-21** — 去除停顿：智能菜单新增浏览器本地 Silero VAD 人声检测，可预览并勾选长停顿，同步精简选中主画面视频及原声。支持停顿阈值、保留间隔、波纹编辑、取消和撤销，界面直接覆盖 13 种语言。
- **2026-09-20 — 播放头标记吸附：** 拖动白色播放头时实时预览画面，靠近标记点或范围两端自动吸附，并显示共享对齐线和时间；按住 Alt 可自由拖动。 标记详情未修改时显示“完成”，修改后显示“应用修改”。
- **2026-09-17 — WebMCP 精剪与审看：**浏览器工具增至 21 个，支持 26 种先审阅再应用的编辑操作，补齐定时片段移动与源范围裁剪、画中画变换、字幕样式与位置、工程画幅设置。Agent 可获取真实渲染画面和短段混音，运行带进度与取消的浏览器本地配音、语音识别任务，并在插入时间线前审阅结果。新增控件与文案覆盖全部 13 种界面语言。
- **2026-09-15 — 导出编辑后的音频：**音频片段导出现在按裁剪范围渲染，并应用播放速度、音量、淡入淡出和空间效果。单个片段与完整时间线混音均可导出为 WAV 或 MP3，视频导出旁新增纯音频导出选项。
