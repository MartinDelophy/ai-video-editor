...
- **2026年8月31日 — 大容量動画の準備を高速化：** ローカル読み込みは端末性能に合わせたフィルムストリップを生成します。素材ライブラリのオンライン動画は実際のダウンロード進捗を先に表示し、その後フレーム準備へ切り替えます。正確なサムネイルは現在の表示範囲を優先し、範囲外は中点分割の順で段階的に補完します。再生ヘッド下のセルは常にライブプレビューの実フレームに追従します。
