...
- **2026 年 8 月 27 日 — 时间线精剪：** 新增视频片段源区间安全的滑移、同步修剪左右相邻片段的三片段滑动，以及支持方向键逐帧滚动微调的编辑点选择。所有模式都保持主画面轨无缝、遵守轨道锁定与源素材边界、提供实时拖拽反馈，并完整本地化到 11 种界面语言。竖屏与 9:16 时间线缩略图现在居中裁切铺满，缩略图单元不再出现上下或左右黑边。
- **2026 年 8 月 26 日 — 感知锁定的波纹剪辑：** 新增时间线工具栏波纹模式。把素材拖到主轨时会先展开与素材时长一致的插入槽，并让后续片段和转场切点同步后移。主画面片段新增片头滚动边界手柄，画中画拖放也会在目标行按真实落点和素材时长显示占位。主视觉插入、复制、删除和时长调整会按相同时间差移动符合条件的定时片段，保持仍在关联的字幕与音频同步，让关联原声继续由主视觉序列驱动，并跳过所有锁定轨道。
- **2026 年 8 月 26 日 — 可移植画面分割：** 修复 Agent `visual.split` 工程，确保左右连续片段在重新打开后继续引用归档源媒体，并在浏览器导出时保留完整视频和内嵌源音频。
