...
- **2026-09-14 — 导出修复与失败重试：**使用纯色背景与透明度关键帧的片段即使没有人物蒙版，也能正常导出。导出失败时保留错误信息，并可按原设置重试。
- **2026-09-14 — 大工程性能优化：**导入工程减少主线程阻塞，缓存音轨分轨计算、精简波形渲染并减少无效音频跳转，降低拖动播放头和播放时的开销。保留全部素材、片段时序、编辑操作、效果与导出行为。导入时显示真实进度，媒体恢复后即可编辑，缩略图继续在后台完善。
